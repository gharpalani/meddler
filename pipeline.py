...
def _is_ingredient(term: str) -> bool:
    """True only if this name is itself an active ingredient.

    Why this exists: querying /drugs for "omeprazole" returns product concepts,
    including an aspirin/omeprazole combination, and attributing that pack's
    ingredients to the patient produced a bleeding-risk warning for someone not
    taking aspirin. Ranking by term type did not help, because /drugs returned
    no IN concept to rank to the front.

    Why it is two calls: /rxcui returns an exact name match regardless of term
    type, so "Norco" comes back with an RxCUI just as "omeprazole" does. An
    earlier version trusted that first call and treated every brand name as its
    own ingredient, which silently disabled duplication detection for Norco,
    Percocet, Tylenol and Tums. The second call reads the concept's actual term
    type and is the only thing that answers the question we are really asking.
    """
    try:
        r = requests.get(f"{RXNAV}/rxcui.json",
                         params={"name": term, "search": 0}, timeout=TIMEOUT)
        ...
        p = requests.get(f"{RXNAV}/rxcui/{ids[0]}/property.json",
                         params={"propName": "TTY"}, timeout=TIMEOUT)
        if p.status_code != 200:
            return False
        props = (p.json().get("propConceptGroup", {}) or {}).get("propConcept") or []
        return any(c.get("propValue") == "IN" for c in props)
