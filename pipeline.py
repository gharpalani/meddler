...
def resolve_ingredients(term: str) -> tuple[list[str], str | None]:
    ...
    Known limitation: a query for a single ingredient can still match a
    combination product containing it, so that product's other ingredients may
    be attributed to the patient. Evaluation found one instance of this across
    15 profiles (omeprazole matching an aspirin/omeprazole product). A fix that
    checked RxNorm term type directly was attempted and reverted, because it
    treated brand names as their own ingredient and silently disabled
    duplication detection. Highest-priority defect for the next iteration.
...
EXTRACT_PROMPT = """Look at this image of medication packaging or a medication list.

List every distinct medication name you can read, one per line.
Copy the text exactly as printed. Do not correct spelling, expand abbreviations,
or add drugs you infer might be present.
If any text is blurred or partly hidden, write it followed by (UNCLEAR).
If you can read no medication names, write NONE."""


def extract_medications_from_image(image_bytes: bytes, mime_type: str,
                                   api_key: str) -> tuple[list[str], str | None]:
    """Read medication names off a photo.

    Vision output is a DRAFT, never a fact. Misreading a drug name is exactly
    the failure this project exists to prevent, so the caller must resolve each
    candidate and have the user confirm before anything is checked. The camera
    is a faster way to fill the text box, not a way to skip it.

    Returns (candidate names, error message or None).
    """
    import base64

    model = pick_model(api_key)
    if not model:
        return [], "Could not reach the model service."
    ...
    try:
        ...
        if r.status_code != 200:
            return [], f"Image service returned {r.status_code}."
        text = r.json()["candidates"][0]["content"]["parts"][0]["text"]
