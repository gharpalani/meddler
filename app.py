...
with st.expander("📷  Add from a photo of your medications"):
    if not key:
        ...
        if found:
            ...
            # No st.rerun() here. This block runs above the picker, so writing
            # to med_picker now is read by the widget further down in the same
            # pass. Calling rerun instead re-registers the widget and drops the
            # additions.
            if c1.button("Add these to my list", type="primary",
                         disabled=not usable):
                for name in usable:
                    if name not in st.session_state.med_picker:
                        st.session_state.med_picker.append(name)
                st.session_state.pop("photo_read", None)
                st.session_state.pop("result", None)
                st.success(f"Added {len(usable)} to your list below.")
